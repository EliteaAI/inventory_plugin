...
class EntityExtractor:
    ...
    def extract(
        ...
        source_toolkit = document.metadata.get('source_toolkit', 'unknown')
    ...
    def extract_batch(
        ...
        Uses parallel processing to extract from multiple chunks concurrently,
        significantly improving throughput for multi-chunk files.
        ...
        if len(documents) > 1 and self.max_parallel_chunks > 1:
            with ThreadPoolExecutor(max_workers=self.max_parallel_chunks) as executor:
                future_to_doc = {
                    executor.submit(self.extract, doc, schema): doc
                ...
                for future in as_completed(future_to_doc):
                    ...
                    try:
                        ...
                        if skip_on_error:
                            logger.warning(f"Skipping document '{file_path}' due to unexpected error: {e}")
                            failed_docs.append(file_path)
                        else:
                            raise RuntimeError(f"Extraction failed for '{file_path}': {e}") from e
        else:
            # Sequential fallback for single document or when parallelization is disabled
            for doc in documents:
                try:
